...
function ClearedTodoItem({ todo, categories }) {
    ...
    return (
        <View className="p-3 mb-2 bg-green-50 dark:bg-gray-900 border border-green-200 dark:border-green-700 rounded-lg">
        <Text> className="text-gray-900 dark:text-gray-100 font-medium">{todo.text}</p>
        <Text> className="text-xs text-gray-500 dark:text-gray-400">Category: {categoryName}</p>
        <Text> className="text-xs text-gray-500 dark:text-gray-400">Cleared: {new Date(todo.clearedAt).toLocaleString("es-ES")}</p>
        </View>
    );
    }
    ...
    return (
        ...
        <View className="bg-green-50 dark:bg-gray-900 p-4 rounded-lg border border-green-200 dark:border-green-700">
            <Text> className="text-sm text-gray-600 dark:text-gray-400">Cleared Todos</p>
            <Text> className="text-xl font-bold text-green-600 dark:text-green-400">{filteredTodos.length}</p>
        </View>
        ...
        <View className="space-y-3">
            ...
            <View className="text-center py-12">
                <Text> className="text-gray-500 dark:text-gray-400 text-lg">There are no cleared Todos. 🎉</p>
            </View>
            ) : (
